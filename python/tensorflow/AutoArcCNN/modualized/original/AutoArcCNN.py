...
class AutoArc2DCNN():
    ...
    def construct_2dcnn(
        ...
        kernels.append(tf.Variable(tf.random_normal([kernel_size, kernel_size, 1, nodeno] ,stddev=1 ) ) ) # first layer
        # kernels.append(tf.Variable(tf.zeros([kernel_size, kernel_size, 1, nodeno] ) ) ) # first layer
        ...
        for i in range(1, layer_no): # create convlution layers (2~last layers)
            ...
            for j in range(0, nodeno):
                kernels[i].append(tf.Variable(tf.random_normal([kernel_size, kernel_size, nodeno, 1] ,stddev=1 ) ) )
                cl_biases[i].append(tf.Variable( tf.zeros([1] )))
        pass
        
        fc_nodeno = 2000
        fc_weights.append(tf.Variable(tf.random_normal([input_dim[0] * input_dim[1] * nodeno, fc_nodeno] ,stddev=1 ) )) # first fully connect layer
        # fc_weights.append(tf.Variable(tf.zeros([input_dim[0] * input_dim[1] * nodeno, nodeno] ) ) ) # first fully connect layer
        fc_biases.append(tf.Variable( tf.zeros([fc_nodeno] ))) # bias for first fully connect layer
        for i in range(1, 3): # create fully connect layers
            fc_weights.append(tf.Variable(tf.random_normal([fc_nodeno, fc_nodeno] ,stddev=1 ) ))
            # fc_weights.append(tf.Variable(tf.zeros([nodeno, nodeno] ) ) )
            fc_biases.append(tf.Variable( tf.zeros([fc_nodeno] )))
        pass
        
        # out_weights = tf.Variable(tf.random_normal([nodeno, output_dim]))
        out_weights = tf.Variable(tf.zeros([fc_nodeno, output_dim]))
        ...
        layers.append(self.conv2d(x, kernels[0], cl_biases[0]))
        current_construct_layer_no += 1
        for i in range(1, layer_no):
            layers.append(tf.concat( [ self.conv2d(layers[i-1], kernels[i][j], cl_biases[i][j] ) for j in range(0, nodeno) ], 
                                     axis=3
                                    )
        ...
        layers.append(self.fc(reshaped_feature_mapes, fc_weights[0], fc_biases[0]))
        current_construct_layer_no += 1
        
        for i in range(1, 3):
            layers.append(self.fc(layers[current_construct_layer_no - 1], fc_weights[i], fc_biases[i]))
            current_construct_layer_no += 1
        pass
    ...
    def TDCNN_run (
        ...
        with tf.Session(config = tfconfig) as sess:
            ...
            print("Initial loss :{}".format(c_loss))
